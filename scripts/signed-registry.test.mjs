...
function fixture(){let revision=0;const db=new Map([['bittrees:roles',JSON.stringify({[actor.address.toLowerCase()]:[{label:'Associate'}]})]]),nonces=new Set();let failed=false;const command=async c=>{if(failed)throw Error('outage');if(c[0]==='MGET')return {result:[String(revision),...c.slice(2).map(k=>db.get(k)??null)]};assert.equal(c[0],'EVAL');if(nonces.has(c[4]))return {result:-2};if(Number(c[6])!==revision)return {result:-1};for(const [k,v] of JSON.parse(c[7]))db.set(k,v);nonces.add(c[4]);return {result:++revision};};return {command,db,get revision(){return revision},fail(){failed=true}};}
async function envelope(endpoint,payload,revision){const e={version:2,audience:'https://gov.bittrees.org',chainId:1,endpoint,address:actor.address,nonce:crypto.randomUUID().replaceAll('-','').repeat(2),timestamp:Date.now(),expectedRevision:revision,payload};return {...e,signature:await actor.signMessage({message:actionMessage(e)})};}
...
test('storage failure and corrupt snapshots are unavailable rather than empty',async()=>{const f=fixture();f.fail();assert.equal((await call(community,f,{},'GET')).code,503);const g=fixture();g.db.set('bittrees:roles','invalid');assert.equal((await call(community,g,{},'GET')).code,503);});
test('wrong audience, route, stale timestamp, ambiguous action and legacy signatures fail',async()=>{for(const change of [{audience:'https://research.bittrees.org'},{endpoint:'/api/rooms'},{timestamp:Date.now()-600000},{payload:{assignRole:{target,label:'Partner'},unassignRole:{target,label:'Partner'}}}]){const f=fixture(),b=await envelope('/api/community',{assignRole:{target,label:'Partner'}},0);Object.assign(b,change);assert.equal((await call(community,f,b)).code,400);assert.equal(f.revision,0)}assert.equal((await call(community,fixture(),{address:actor.address,signature:'0x',timestamp:Date.now()})).code,400)});
