...
test('connected wallets export local data without enabling messaging',async({browser,baseURL},info)=>{
 ...
 try{
  ...
  const page=await context.newPage();await page.goto('/messenger');
  ...
  await expect(page.getByRole('button',{name:'Enable direct messages',exact:true})).toBeVisible();
  await page.locator('summary').filter({hasText:'Export local data to Chat'}).click();
  ...
  for(const shared of [false,true]){
   ...
   expect(data.source).toBe('governance');expect(data.contacts).toEqual(JSON.parse(original[`bittrees.contacts.${owner}`]));expect(data.notes).toEqual(JSON.parse(original[`bittrees.dm.saved.${owner}`]));
